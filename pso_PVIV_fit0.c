 ...
 switch(model)
  { case '1':
    ...
    default:
     ...
     printf("\n After iterating %d times, the fitness value is: %.7e.\n", best_gen_number, best);
   ...
   /* replace the initial parameters with the best fit */
     switch(model)
          ...
          printf("\t Jph= %7.3f[mA/cm^2], Js= %7.3e[mA/cm^2],\n\t Rs= %7.3e[Ohm/cm^2], Rp= %9.3e[Ohm/cm^2], n= %f5.3 \n",
   ...
   /* Save fitting curve into a file */
    ...
    printf("\n #- Program running time: %lf seconds -# \n",duration);
